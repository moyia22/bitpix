...
class S3Storage implements PrivateStorage {
  ...
  async put(key: string, data: Buffer, contentType: string): Promise<void> { await this.client.send(new PutObjectCommand({ Bucket: this.bucket, Key: key, Body: data, ContentType: contentType, ServerSideEncryption: env.S3_SERVER_SIDE_ENCRYPTION })); }
